...
export const AnalysisPassingView: React.FC<AnalysisPassingViewProps> = ({ setActiveTab }) => {
  ...
  if (!summary) {
    return (
      <div className="p-12 text-center bg-white dark:bg-[#000000] rounded-3xl border border-slate-100">
        ...
        <button onClick={() => setActiveTab('tryouts')} className="mt-4 px-5 py-2 rounded-xl bg-[#FF6B6B] text-white text-xs font-bold">Mulai Tryout</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-16 animate-in fade-in duration-300">
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-[#000000] border border-slate-100 dark:border-[#141414] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
          <button
            ...
            className="px-5 py-3 rounded-2xl bg-indigo-50 dark:bg-indigo-950/50 text-[#FF6B6B] hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-xs font-bold transition-all flex items-center justify-center gap-2 border border-indigo-200 dark:border-indigo-800"
          >
      ...
      `}>
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/15 text-xs font-extrabold backdrop-blur-md uppercase tracking-wider">
            ...
            {!summary.is_passed_total && (
              <div className="pt-2 flex flex-wrap gap-3">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-[#FF6B6B] text-white text-xs font-bold hover:bg-[#E85D5D] shadow-lg transition-all flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-white/20 hover:bg-white/30 text-white text-xs font-bold backdrop-blur-sm transition-all flex items-center gap-2"
                >
      ...
      <div className="p-6 lg:p-8 rounded-3xl bg-white dark:bg-[#000000] border border-slate-100 dark:border-[#141414] shadow-xs space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-[#141414] pb-4">
          ...
          <span className="text-xs font-semibold px-3 py-1 rounded-xl bg-slate-100 dark:bg-[#000000] text-slate-600 dark:text-slate-300 self-start sm:self-auto">
              ...
              <div key={gapItem.competency_code} className="p-4 sm:p-5 rounded-2xl bg-slate-50 dark:bg-[#000000] border border-slate-100 dark:border-[#141414]/80 transition-all hover:border-indigo-200">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    <span className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs shrink-0 ${isSafe ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300'}`}>
                  ...
                  <div className="flex items-center gap-4 self-end sm:self-auto">
                    ...
                    <div className={`px-3 py-1 rounded-xl text-xs font-extrabold flex items-center gap-1 min-w-[80px] justify-center ${isSafe ? 'bg-emerald-500 text-white shadow-xs' : 'bg-rose-500 text-white shadow-xs animate-pulse'}`}>
        ...
        {summary.weakest_competency && (
          <div className="p-6 rounded-3xl bg-linear-to-r from-[#FF6B6B]/10 to-[#4D5DFB]/10 border border-[#FF6B6B]/20 dark:border-[#FF6B6B]/30 flex flex-col sm:flex-row items-center justify-between gap-4 mt-8">
            ...
            <button
              onClick={() => setActiveTab('recom_tutors')}
              className="px-5 py-2.5 rounded-xl bg-[#FF6B6B] hover:bg-[#E85D5D] text-white text-xs font-bold shrink-0 shadow-md transition-all"
            >
